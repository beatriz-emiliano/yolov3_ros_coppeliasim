...
if false then -- if not sim.isPluginLoaded('Bwf') then
    ...
    function sysCall_init()
        ...
        sim.writeCustomDataBlock(sim.getObject('.'),simBWF.modelTags.TESTMODEL,sim.packTable({version=1})) -- append the tag with data that just contains the version number
        model={}
